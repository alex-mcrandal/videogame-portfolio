/*
 * MatchMakingService is a helper class that provides additional functionality with Lobbies.
 */

/*
 * Special thanks to Matthew Spencer for sharing this open-source code.
 */
...
namespace LobbyHelpers
{
    public static class MatchMakingService
    {
        ...
        public static async Task CreateLobbyWithAllocation(LobbyData _data)
        {
            // Create a relay allocation and generate a join code to share with the lobby
            var a = await RelayService.Instance.CreateAllocationAsync(_data.maxPlayers);
            ...
            var options = new CreateLobbyOptions
            {
                ...
                { Constants.joinKey, new DataObject(DataObject.VisibilityOptions.Member, joinCode) },
                {
                    ...
                    new DataObject(DataObject.VisibilityOptions.Public, _data.difficulty.ToString(), DataObject.IndexOptions.N2)
                }
            }
            };

            _currentLobby = await Lobbies.Instance.CreateLobbyAsync(_data.name, _data.maxPlayers, options);
